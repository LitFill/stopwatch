...
const formatTimeShort = time => {
    ...
    formattedTime += `${seconds < 10 ? '0' + seconds: seconds}.${milliseconds < 10 ? '0' + milliseconds: milliseconds}`;
...
const displayLapTimes = () => {
    ...
    lapTimes.forEach((lapTime, index) => {
        if (index > 0) {
            ...
            text += `\n${index + 1}. ${formatTime(lapTime)} (+${formatTimeShort(lapTimeDiff)})`;
        } else {
            text += `\n${index + 1}. ${formatTime(lapTime)} (+${formatTimeShort(lapTime)})`
        }
    });
    lapTimeElement.innerText = text;
};
